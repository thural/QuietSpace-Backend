...
@pytest.mark.asyncio
async def test_update_comment_requires_auth(client: AsyncClient):
...
async def test_get_comment_replies(client: AsyncClient):
    response = await client.get(
        "/api/v1/comments/00000000-0000-0000-0000-000000000000/replies"
    )
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
...
@pytest.mark.asyncio
async def test_get_comments_by_user_empty(client: AsyncClient):
    response = await client.get("/api/v1/comments/user/00000000-0000-0000-0000-000000000001")
    assert response.status_code == 200
    data = response.json()
    assert data["items"] == []
